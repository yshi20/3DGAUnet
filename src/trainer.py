...
def trainer(args):
    ...
    save_file_path = paramt.output_dir + '/' + args.model_name
    ...
    if args.logs:
        ...
        # writer = SummaryWriter(paramt.output_dir+'/'+args.model_name+'/'+model_uid+'_'+args.logs+'/logs')
        writer = SummaryWriter(paramt.output_dir + '/' + args.model_name + '/' + args.logs + '/logs')

        image_saved_path = paramt.output_dir + '/' + args.model_name + '/' + args.logs + '/images'
        model_saved_path = paramt.output_dir + '/' + args.model_name + '/' + args.logs + '/models'
    ...
    #dsets_path = paramt.data_dir + paramt.model_dir + "30/train/"
    dsets_path = paramt.data_dir + paramt.model_dir
    # if paramt.cube_len == 64:
    #     dsets_path = paramt.data_dir + paramt.model_dir + "30/train64/"
    ...
    train_dset_loaders = torch.utils.data.DataLoader(train_dsets, batch_size=paramt.batch_size, shuffle=True,
                                                     num_workers=4)
    ...
    D_solver = optim.Adam(D.parameters(), lr=paramt.d_lr, betas=paramt.beta)
    # D_solver = optim.SGD(D.parameters(), lr=args.d_lr, momentum=0.9)
    G_solver = optim.Adam(G.parameters(), lr=paramt.g_lr, betas=paramt.beta)
    ...
    for epoch in range(paramt.epochs):
        ...
        for phase in ['train']:
            ...
            for i, X in enumerate(tqdm(dset_loaders[phase])):
                ...
                real_labels = torch.ones_like(d_real).to(paramt.device)
                fake_labels = torch.zeros_like(d_fake).to(paramt.device)
                # print (d_fake.size(), fake_labels.size())

                if paramt.soft_label:
                    real_labels = torch.Tensor(batch).uniform_(0.7, 1.2).to(paramt.device)
                    fake_labels = torch.Tensor(batch).uniform_(0, 0.3).to(paramt.device)
                ...
                d_total_acu = torch.mean(torch.stack((d_real_acu, d_fake_acu), 0))

                if d_total_acu < paramt.d_thresh:
                    D.zero_grad()
                    d_loss.backward()
                    D_solver.step()
                ...
                # g_loss = recon_g_loss + paramt.adv_weight * adv_g_loss
                g_loss = adv_g_loss
            ...
            if (epoch + 1) % paramt.model_save_step == 0:
                ...
                #print (samples.shape)
                #for i in range(0,7):
                #    name = './images/train_panc_'+str(epoch)+'_'+str(i)+'.mat' 
                #    io.savemat(name, {"instance": samples[i,:,:,:]})
